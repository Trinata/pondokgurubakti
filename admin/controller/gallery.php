...
var $models = FALSE;
		...
		global $CONFIG;
			...
			if ($_POST == null){	
				...
				if ($_FILES['cover_gambar']['name']) {
					...
					$filename=$_POST['cover_gambar1'];
				}
				
				$data=$this->models->updatealbum($id_gmb, $judul, $deskripsi, $filename, $status);		
					if($data == 1){
						echo "<script>alert('Data berhasil disimpan');window.location.href='".$CONFIG['admin']['base_url']."gallery'</script>";
					}
			}	
	}
	...
	public function deletealbum(){
		...
		$data=$this->models->deletealbum($id_gmb);
		if($data == 1){
	...
	public function deletegallery(){
		...
		if($data == 1){
			echo "<script>alert('Data berhasil di hapus');window.location.href='".$CONFIG['admin']['base_url']."gallery/getgallery/?id_album=".$id_album."'</script>";
		}
		else {pr('gagal');}
	}
}
